...
class HCPTexture:
    ...
    def fromEulerAnglesFile(self, fname, cols=(0, 1, 2), conv='passive', units='rad'):
        ...
        conv = 'active' or 'passive' (for HCPGrain object)
        ...
        key = 0
        for eulers in data:
            self.orientDict[key] = HCPGrain(conv=conv, ortho=self.has_ortho_symm)
    ...
    def toEulerAnglesFile2(self, fd, out='rad', load_vec=RD):
        ...
        for idx,theta in enumerate(self.orientDict):
            ...
            #min_nd_rotated = cur_orient.getAlignment(HCPGrain.families[100], ND)
            #min_planar_double = cur_orient.getAlignment(HCPGrain.families[111], TD)
            if (phi1, PHI, phi2) != (None, None, None):
                if out == 'rad':
    ...
    def toPointPFFiles(self, fd, family, spec_axis, num_atts=0):
        '''
        writes the pole figure coordinates for each HCPGrain object
        for the specified family and specimen axis
        family = HCPGrain.families[family]
        ...
        for theta in self.orientDict:
            ...
            data_pts = cur_orient.getPoleFigureCoords(HCPGrain.families[family], spec_axis)
...
#                    r1,r2,r3 = self.orientDict[theta].asRodrigues()
    ...
    def generateTexture(self, step_size, step_num, type='2d', out='rad', conv='active'):
        # TODO: This needs to be changed to HCP
        '''
        type=
        ...
        for theta in xrange(step_num):
            theta *= step_size
            self.orientDict[theta] = HCPGrain(conv, ortho=self.has_ortho_symm)
            if type == '2d':
        ...
        for i in range(int(2 * pi / resolution)):
            ...
            for j in range(int(pi / resolution)):
                ...
                for k in range(int(2 * pi / resolution)):
                    phi2 = k * resolution
                    self.orientDict[key] = HCPGrain(conv, ortho=self.has_ortho_symm)
    ...
    def generateFCCOrthoRandom(self, resolution=10., out='rad', conv='active'):
        '''
        generate a random texture by creating a structured grid of points
        that fills a single HCP-Orthotropic Euler subspace
        '''
    ...
    def mimicRolled(self, binsize=pi / 12., resolution=20):
        ...
        for key in random.orientDict.keys():
            ...
            # ND rotated cube?
            for crystal_vec in HCPGrain.families[100]:
                rotated_vec = dot(cur_orient.R, crystal_vec)
                distance = fabs(HCPGrain._getAngleBetween(rotated_vec, ND))
            ...
            if min_distance > binsize:
                for crystal_vec in HCPGrain.families[111]:
                    rotated_vec = dot(cur_orient.R, crystal_vec)
                    distance = fabs(HCPGrain._getAngleBetween(rotated_vec, TD))
                    if distance > pi / 2: distance = pi - distance
                    if distance < min_distance:
                        min_distance = distance
                        if min_distance < binsize:
    ...
    def _ZhaiPaperStudy(self, n):
        ...
        for cur_theta in self.orientDict:
            O2_traces = HCPGrain.planeTraces(self.orientDict[cur_theta].R, GB)
            alpha = []
            for O2_trace in O2_traces:
                tmp = fabs(HCPGrain._getAngleBetween(O1_trace, O2_trace))
